...
from catalog.notifications.emailer import send_admin_new_customer_alert, send_welcome_email
...
async def register(body: UserCreate, db=Depends(get_db)):
    ...
    try:
        send_admin_new_customer_alert(created)
    except Exception as e:
        logger.error("Admin new-customer alert failed for %s: %s", created["email"], e)
...
async def google_auth(body: GoogleAuthIn, db=Depends(get_db)):
    ...
    if user is None:
        ...
        try:
            send_admin_new_customer_alert(user)
        except Exception as e:
            logger.error("Admin new-customer alert failed for %s: %s", user["email"], e)
